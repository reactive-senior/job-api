const client = require('../models/VendorAuthModel');
...
exports.doSignUp = function(req, res){
	...
	var query=`INSERT INTO public.party(type, password, username)
								VALUES (\'001\', \'${md5_pass}\', \'${req.body.username}\') RETURNING *`;
...
exports.doLogIn = function(req, res){
	var md5_pass = md5(req.body.password);
	var query = `SELECT * FROM public.party WHERE username=\'${req.body.username}\' AND password=\'${md5_pass}\' AND type=\'001\'`;
	client.query(query, (err, response)=>{
		if(err){
